...
@interface MenuEditTableSnapShotImageView : UIImageView
...
@implementation MenuEditTableSnapShotImageView
...
@interface MenuEditTableView ()

@property (nonatomic, assign) CGPoint touchOffset;
@property (nonatomic, strong) MenuEditTableSnapShotImageView *snapShotImageView;
...
@interface MenuEditTableView (AutoscrollingMethods)

- (void)maybeAutoscrollForSnapShotImageView:(MenuEditTableSnapShotImageView *)snapShot;
...
@implementation MenuEditTableView
...
- (void)setup
{
    ...
    movingGestureRecognizer.minimumPressDuration = .60;
...
- (void)moveRowToLocation:(CGPoint)location
{
    ...
    if ([newIndexPath section] != NSNotFound && [newIndexPath row] != NSNotFound && [newIndexPath compare:[self movingIndexPath]] != NSOrderedSame)
    {
        ...
        [self beginUpdates];
        NSLog(@"moveRowToLocation: %ld", (long)[self movingIndexPath].row);
        
        NSLog(@"moveRowToLocation: %ld", (long)newIndexPath.row);
        
        // Move the row
        [self deleteRowsAtIndexPaths: [NSArray arrayWithObject: [self movingIndexPath]]
...
{   gestureRecognizer.minimumPressDuration = .60;
    switch ([gestureRecognizer state])
    {
        case UIGestureRecognizerStateBegan:
        {
            CGPoint touchPoint = [gestureRecognizer locationInView:self];
            if(touchPoint.x < 288.0) {
                [gestureRecognizer cancelTouch];
                return;
            }
            
            // Grab the touched index path
            NSIndexPath *touchedIndexPath = [self indexPathForRowAtPoint:touchPoint];
            ...
            BLRSelectedPlayerCell *touchedCell = (BLRSelectedPlayerCell *)[self cellForRowAtIndexPath:touchedIndexPath];
            ...
            MenuEditTableSnapShotImageView *snapShotOfMovingCell = [[MenuEditTableSnapShotImageView alloc] initWithImage:image];
            ...
            [UIView animateWithDuration:0.2
                             ...
                             completion:^(BOOL finished) {
                                 ...
                                 if (finished)
                                 {
                                     ...
                                     [self reloadRowsAtIndexPaths: [NSArray arrayWithObject: movingIndexPath] withRowAnimation: UITableViewRowAnimationFade];
                                 }
...
{
    [self reloadSections: [[NSIndexSet alloc] initWithIndex: section]
        withRowAnimation: UITableViewRowAnimationFade];
}
...
- (void)autoscrollTimerFired:(NSTimer *)timer
{
    ...
    MenuEditTableSnapShotImageView *snapShot = (MenuEditTableSnapShotImageView *)[timer userInfo];
    [snapShot moveByOffset: CGPointMake(0, [self autoscrollDistance])];
    
    [self moveRowToLocation: [[self movingGestureRecognizer] locationInView: self]];
}
...
- (void)setSnapShotImageView:(MenuEditTableSnapShotImageView *)snapShotImageView
{
